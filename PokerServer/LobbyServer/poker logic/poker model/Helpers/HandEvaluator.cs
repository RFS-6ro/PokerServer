namespace TexasHoldem.Logic.Helpers
{
    ...
    public class HandEvaluator : IHandEvaluator
    {
        ...
        public BestHand GetBestHand(IEnumerable<CardData> cards)
        {
            ...
            if (cardSuitCounts.Any(x => x >= ComparableCards))
            {
                ...
                for (var i = 0; i < cardSuitCounts.Length; i++)
                {
                    ...
                    {
                        var flushCards =
                            cards.Where(x => x.Suit == (int)(CardSuit)i)
                                .Select(x => x.Type)
            ...
            if (cardTypeCounts.Any(x => x == 4))
            {
                ...
                var bestCards = new List<SafeInt>
                                    {
                                        bestFourOfAKind,
                                        bestFourOfAKind,
                                        bestFourOfAKind,
                                        bestFourOfAKind,
                                        cards.Where(x => x.Type != (int)bestFourOfAKind).Max(x => x.Type),
                                    };
            ...
            {
                var bestCards =
                    cards.Where(x => x.Type != (int)pairTypes[0])
                        .Select(x => x.Type)
                        .OrderByDescending(x => x)
                        .Take(3).ToList();
                bestCards.Add((int)pairTypes[0]);
                bestCards.Add((int)pairTypes[0]);
        ...
        private IList<SafeInt> GetTypesWithNCards(int[] cardTypeCounts, int n)
        {
            var pairs = new List<SafeInt>();
            for (var i = cardTypeCounts.Length - 1; i >= 0; i--)
            {
                if (cardTypeCounts[i] == n)
                {
                    pairs.Add((int)(CardType)i);
                }
            }
        ...
        private ICollection<SafeInt> GetStraightFlushCards(int[] cardSuitCounts, IEnumerable<CardData> cards)
        {
            var straightFlushCardTypes = new List<SafeInt>();
            for (var i = 0; i < cardSuitCounts.Length; i++)
            {
                if (cardSuitCounts[i] < ComparableCards)
                {
        ...
        private ICollection<SafeInt> GetStraightCards(int[] cardTypeCounts)
        {
            ...
            for (var i = cardTypeCounts.Length - 1; i >= 1; i--)
            {
                ...
                if (hasCardsOfType && i == lastCardType - 1)
                {
                    ...
                    {
                        var bestStraight = new List<SafeInt>(ComparableCards);
